print('declaration_11.3.R'); library(DeclareDesign); library(rdddr); library(tidyverse)
...
declaration_11.3 <-
  ...
  declare_estimator(handler = function(data) {
    map(polynomial_degrees, ~lm(Y ~ poly(X, .), data = data)) |> 
      ...
      map_dfc(~predict(., newdata = tibble(X = x_range))) |> bind_cols(tibble(X = x_range)) |> mutate(inquiry = str_c("X_", X)) |> 
